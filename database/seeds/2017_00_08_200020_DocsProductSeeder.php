	...
	public function run() {
		...
		Models\Product::build(function (Builder $b) {
			$b->ent_id($this->entId)->code('11')
				->title("阿米巴巴套件实施手册")
				->cover('/assets/vendor/suite-docs/cover/1.png')
		...
		Models\Product::build(function (Builder $b) {
			$b->ent_id($this->entId)->code('12')
				->title("阿米巴巴套件帮助文档")
				->cover('/assets/vendor/suite-docs/cover/2.png')
				->summary('想获得更多相关信息，请参阅帮助文档!');
		});
		Models\Product::build(function (Builder $b) {
			$b->ent_id($this->entId)->code('13')
				->title("阿米巴巴套件接口开发手册")
				->cover('/assets/vendor/suite-docs/cover/3.png')
		...
		Models\Product::build(function (Builder $b) {
			$b->ent_id($this->entId)->code('14')
				->title("阿米巴巴套件API说明")
				->cover('/assets/vendor/suite-docs/cover/4.png')
				->summary('详细描述套件API信息');
		});
		Models\Product::build(function (Builder $b) {
			$b->ent_id($this->entId)->code('15')
				->title("阿米巴巴套件速查表")
				->cover('/assets/vendor/suite-docs/cover/5.png')
				->summary('阿米巴巴套件实体信息查询');
		});
		Models\Product::build(function (Builder $b) {
			$b->ent_id($this->entId)->code('16')
				->title("阿米巴巴套件开发手册")
				->cover('/assets/vendor/suite-docs/cover/6.png')
				->summary('阿米巴巴套件实体信息查询');
		});
	}
}
